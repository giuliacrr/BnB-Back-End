<form action="{{ $action }}" method="POST" enctype="multipart/form-data">
  ...
  <div class="mb-3">
    ...
    <input type="number" min="1" name="rooms_number" id="rooms_number"
      ...
      class="form-control @error('rooms_number') is-invalid @enderror" oninput="checkLength('rooms_number')" required>
    @error('rooms_number')
    <div class="invalid-feedback">{{ $message }}</div>
    @enderror
    {{-- beds number --}}
    ...
    <input type="number" min="1" name="beds_number" id="beds_number"
      ...
      class="form-control @error('beds_number') is-invalid @enderror" oninput="checkLength('beds_number')" required>
    @error('beds_number')
    <div class="invalid-feedback">{{ $message }}</div>
    @enderror
    {{-- bathrooms number --}}
    ...
    <input type="number" min="1" name="square_meters" id="square_meters"
      ...
      class="form-control @error('square_meters') is-invalid @enderror" oninput="checkLength('square_meters')" required>
    @error('square_meters')
    <div class="invalid-feedback">{{ $message }}</div>
    @enderror
    {{-- city --}}
    <label class="form-label fw-bold">Città:</label>
    <input type="text" name="city" value="{{ old('city', $apartment?->city) }}"
              ...
              <div class="d-flex flex-wrap">
                ...
                <div class="form-check m-1">
                  <input class="form-check-input" type="checkbox" id="service{{ $service->id }}" name="services[]"
                    value="{{ $service->id }}" {{$apartment?->services->contains($service) || in_array($service->id,
